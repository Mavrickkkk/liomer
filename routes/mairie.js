...
router.get('/mairie', function (req, res) {
    ...
    connection.query('SELECT * FROM compte_rendu ORDER BY date DESC LIMIT 8', (err, rows) => {
        if (err) {
...
router.post('/envoyerCompteRendu', upload.single('file'), function (req, res, next) {
    ...
    connection.query('INSERT INTO compte_rendu (date, filePath) VALUES (?, ?)', [req.body.date_creation, filePath], function (error, results, fields) {
        if (error) {
            res.send(error);
        } else {
            res.redirect('/mairie');
